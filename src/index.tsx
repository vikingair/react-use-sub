...
export type StoreSet<DATA> = <K extends keyof DATA>(
  update: StoreSetArg<DATA, K>,
) => void;
...
export type StoreType<DATA> = {
  get: () => DATA;
  set: StoreSet<DATA>;
  listen: StoreListen<DATA>;
};
...
): boolean => a.length !== b.length || a.some((v, i) => b[i] !== v);
...
const _update = <DATA extends Record<string, unknown>, K extends keyof DATA>(
  D: InternalDataStore<DATA>,
  next: Pick<DATA, K>,
): void => {
  const result = { ...D.data };
...
const _center = <DATA extends Record<string, unknown>>(
  D: InternalDataStore<DATA>,
): StoreType<DATA> => ({
  get: () => D.data,
  set: <K extends keyof DATA>(update: StoreSetArg<DATA, K>) => {
    const next: Pick<DATA, K> | undefined =
      typeof update === "function" ? update(D.data) : update;
    if (next) {
      _update(D, next);
      _config.batch(() => {
