...
import { _config, createStore } from "../src";
...
describe("createStore", () => {
  ...
  it("creates a store that can be updated", () => {
    ...
    // @ts-expect-error invalid property will NOT be ignored
    Store.set({ what: "hip" });
    expect(Store.get()).toEqual({ foo: "hop again", bar: 3, what: "hip" });
    // @ts-expect-error still invalid property
  ...
  it("uses an invalid mapper producing always non comparable results without throwing an error", async () => {
    ...
    const Dummy = () => {
      ++renderCount;
      currentReceived = useSub((store) => ({
        entries: Object.entries(store).concat(lastEntry),
      }));
      return null;
    };
    // when
